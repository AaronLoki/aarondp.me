---
layout: page
title: Study Tools
icon: fas fa-tools
order: 5
---

# Study Tools

Helpful tools and utilities for IB students.

## Academic Tools

### Mathematics

- [Desmos Graphing Calculator](https://www.desmos.com/calculator)
- [Wolfram Alpha](https://www.wolframalpha.com/)
- GeoGebra for geometry and algebra

### Sciences

- Periodic table references
- Unit converters
- Scientific notation calculators
- Physics formula sheets

### Languages

- Online dictionaries
- Grammar checkers
- Translation tools (for reference only)

## Citation & Writing

- [Citation Machine](https://www.citationmachine.net/)
- [Purdue OWL](https://owl.purdue.edu/)
- Bibliography generators
- Plagiarism checkers

## Productivity

### Study Planning

- Pomodoro timers
- Task management apps
- Study schedule templates
- Goal trackers

### Note-Taking

- Digital note apps
- Mind mapping tools
- Flashcard creators
- PDF annotators

## IB-Specific

- CAS tracking tools
- IA planning templates
- EE timeline organizers
- TOK exhibition planners

## Exam Preparation

- Formula sheets by subject
- Command terms glossary
- Mark scheme guides
- Time management calculators

---

*Bookmark this page for quick access to essential study tools!*
